# Architecture — Isotope

## Stack

| Layer          | Technology                                               | Role                                                                |
| -------------- | -------------------------------------------------------- | ------------------------------------------------------------------- |
| Frontend       | TanStack Start (React 19, TypeScript, Vite)              | Full-stack React framework; routing, SSR, Cloudflare Workers deploy |
| Backend / API  | Hono v4 on Cloudflare Workers                            | HTTP API handlers; auth middleware; business logic via Repo layer   |
| Database       | Cloudflare D1 (SQLite) + Drizzle ORM                     | All structured application data — users, companies, contacts, notes |
| Shared types   | `packages/schemas` (Zod v4 + TypeScript)                 | Single source of truth for all types, Zod schemas, and enums        |
| Styling        | Tailwind CSS v4 + CSS custom property tokens + shadcn/ui | UI styling; token system defined in `styles.css`                    |
| Auth           | Clerk (`@clerk/tanstack-react-start`, `@clerk/backend`)  | User sign-up, sign-in, session management, SSO                      |
| Server state   | TanStack Query                                           | Data fetching, caching, and mutation on the frontend                |
| Forms          | TanStack Form                                            | Form state and validation on the frontend                           |
| Icons          | `@phosphor-icons/react`                                  | Icon library                                                        |
| Logging        | `@logtape/logtape` via `AppLogger`                       | Structured logging across worker and DAL layers                     |
| Error tracking | Sentry                                                   | Runtime error capture                                               |
| Testing        | Vitest + React Testing Library                           | Unit and integration tests                                          |

## System Boundaries

- `packages/schemas/src/<feature>/` — Owns all Zod schemas and TypeScript types. The only place where request/response types and enums are defined. Shared by both `apps/web` and `apps/backend`.
- `apps/backend/src/data-access-layer/` — Raw Drizzle DB operations only. No business logic. Every method wraps in try/catch and returns a typed response object.
- `apps/backend/src/repositories/` — Business logic layer. Maps API request shapes to DAL params. The only layer that may call DAL methods.
- `apps/backend/src/routes/` — Hono route handlers. Auth middleware runs first (`checkAuth`), then `zValidator`, then delegates to Repo. Instantiates Repo per request.
- `apps/backend/src/index.ts` — Hono app entry point. Mounts all route groups, configures CORS, request ID, and logger middleware.
- `apps/web/src/routes/_authenticated/` — All authenticated frontend pages and co-located private components. Route layout wrapper enforces Clerk session check.
- `apps/web/src/routes/_authenticated/<feature>/-data.ts` — Frontend data layer per feature. `QueryOptions` classes with hierarchical keys, mutation hooks with explicit `onError` handlers, `apiClient` calls.

## Query Route Convention

Endpoints that are semantically GET (read-only, no side effects) but require a request body for filters, search, or pagination use `POST` on the resource router. All routes for a resource live in a single `<Feature>Routes.ts` file — query routes are registered alongside mutation routes within the same Hono instance.

Pagination fields (`pageNo`, `pageSize`, `sortColumn`, `sortDirection`) are part of `ZGetJobsApiRequest` and default to `pageNo=1`, `pageSize=20`, `sortColumn=createdAt`, `sortDirection=desc` in the Repo layer. The DAL applies `LIMIT`/`OFFSET` and `ORDER BY` at the SQL level.

## Storage Model

- **Cloudflare D1 (primary database)**: All structured data — `users`, `companies`, `contacts`, `contact_history`, `notes`, `jobs`, `tasks`. Every table has a `created_by` column (Clerk user ID) as a scoping key. Multi-tenant by design.
  - `tasks`: Backs the Tasks page (formerly the placeholder Today page — `/today` was renamed to `/tasks`). Each row is a follow-up: `contactId` (nullable FK — schema allows non-contact tasks in future, but V1 only auto-creates contact follow-ups), `channel` (`ContactHistoryChannelEnum`, not nullable — email and linkedin sequences run independently per contact), `title`, `dueAt` (date-only `YYYY-MM-DD` text, not a full timestamp — comparisons are lexical string comparisons), `status` (IntEnum: 1=Pending, 2=Completed, 3=Missed, 4=Paused), `stepNumber` (nullable int), `pausedAt` (nullable), `note` (nullable), `completedAt` (nullable). **NOTE: the rest of this bullet (fixed 7-day interval, `contacts.nextTouchDueAt`, single-row-forever-UPDATE) predates the configurable multi-step sequences feature and is stale — see `context/plans/followup-sequences-plan.md` and its 2026-07-29 addendum for the current design.** As of that addendum: `ContactsRepo.logContactHistory` on an outbound (`Me`) message marks the contact+channel's active (Pending/Paused) task row `Completed` (stamping `completedAt`) and inserts a fresh Pending row for the next step (`TasksDAL.syncFollowUpForContact`, `completePriorStep: true`) — so a contact+channel accumulates one `Completed` row per finished step plus at most one active row; `IDX_tasks_contact_id_channel` is a plain (non-unique) index to allow this. An inbound reply still only pauses the active row, never completes it. Edits/undos of a message's `sentAt` recompute the active row's `dueAt` in place (`completePriorStep: false`) without completing anything. A daily cron (`triggers.crons` in `workers/api/wrangler.jsonc`, `TaskMissedSweepHandler` via the worker's `scheduled()` export) flips `Pending` tasks with `dueAt < today` to `Missed`. Overdue rollup is a query-time concern, not a data concern: `TasksDAL.getTasksForDate` special-cases `date === today` to also pull in any `Pending`/`Missed`/`Paused` task with `dueAt < today`. Past dates only ever show `Completed` tasks (via `GET /tasks/past`, `dueAt < today` — a task completed before its original scheduled `dueAt` won't appear there until that date passes, a known pre-existing gap). The calendar-strip dot query (`POST /tasks/calendar`) reports `hasPending`/`hasCompleted`/`hasMissed`/`hasPaused` per day.
  - `jobs`: Tracks job applications. `status` and `type` stored as integers (IntEnum). `skills` stored as JSON-stringified text array — parsed at application layer. `url` has a composite UNIQUE INDEX on `(url, created_by)` — URLs are deduplicated per user, not globally. `match_score` nullable (reserved for v2.0). FK to `companies.id` via `company_id` (nullable). `status` integer mapping: 1=NotStarted (Manual default), 2=WaitingForHuman (LLM default), 3=Accepted, 4=Applied, 5=CompanyAdded, 6=Interviewing, 7=Offer, 8=Rejected. `type` mapping: 1=Manual, 2=LLM. **List endpoint is `POST /jobs` with `{ searchText?, pageNo?, pageSize?, sortColumn?, sortDirection? }` body** — server-side pagination via `LIMIT`/`OFFSET`, server-side sort via `ORDER BY`. **Count endpoint is `POST /jobs/count` with the same body** — both share identical filter logic so count and list stay in sync. `GET /jobs` does not exist. Search runs SQLite `LIKE` across title, location, salary, and company name (LEFT JOIN on `companies`).
- `job_search_frameworks`: Stores versioned job search criteria per user. `isCustomized` boolean — `false` for the default seed row, `true` once the user saves their own values. Latest version always fetched via `ORDER BY version DESC LIMIT 1`. Old versions pruned to max 5. Array columns (`targetRoles`, `requiredSkills`, `skills`, `preferredLocations`) stored as JSON-stringified text — parsed at DAL layer. Default row seeded via `FrameworksDAL.createDefaultIfAbsent` called from `UsersRepo.syncClerkUser` on every Clerk webhook upsert (idempotent).
- `browser_run_budget`: Single global row (id=1) tracking Browser Run usage against the Workers paid plan's 10-hour/month included quota. `used_seconds` (real) is atomically incremented via `used_seconds + ?` SQL after each scrape. `reset_at` stores the ISO timestamp of the first day of the next UTC month — checked on every read; if `now >= reset_at`, the row is reset before returning. No `created_by` column — this is a system-level resource, not user-scoped. The DAL (`BrowserRunBudgetDAL`) exposes `isShutdown()` (read + auto-reset) and `recordUsage(elapsedSeconds)` (increment). If D1 is unavailable, `isShutdown()` fails open so a DB blip never silently drops jobs.
- **No blob / file storage**: All content (AI summaries, draft bodies, personalization notes, conversation history) stored as text in D1 directly.

## Scheduled Jobs

- The API worker (`workers/api/index.ts`) exports a `scheduled()` handler alongside `fetch()` — the first cron trigger in the codebase. `triggers.crons: ["5 0 * * *"]` (00:05 UTC daily) is set in both `staging` and `production` envs of `workers/api/wrangler.jsonc`.
- Currently the only scheduled job is `TaskMissedSweepHandler.handle(env)` — sweeps `tasks` rows from `Pending` to `Missed` once their `dueAt` has passed (system-wide, no `created_by` scoping, since a cron has no per-request user context — same pattern as the `browser_run_budget` single-row table).
- Cloudflare Workers do not trigger `scheduled()` locally in `wrangler dev` — test manually via `curl "http://localhost:8788/cdn-cgi/handler/scheduled"`.

## Auth and Access Model

- Every user authenticates via Clerk. Clerk issues and manages session tokens.
- All worker API routes run `checkAuth` middleware first; unauthenticated requests receive `401`.
- Every DB table has `created_by` (Clerk user ID). Every query includes a `WHERE created_by = ?` filter — cross-user data access is structurally impossible.
- The `_authenticated` route wrapper in `apps/web` checks `useAuth().isSignedIn`; unauthenticated visitors see a sign-in prompt.
- Worker routes extract `clerkUserId` and `clerkEmail` from the verified Clerk token via `c.get("clerkUserId")`.

## Invariants

1. **Layer import order is strictly enforced: Routes → Repo → DAL → DB.** Routes never call DAL or Drizzle directly. Repos never call Drizzle directly. All data serialization (e.g. `JSON.stringify` for array columns) and DB-level concerns (version calculation, pruning) belong in the DAL, not the Repo.
2. **All types and Zod schemas live exclusively in `packages/schemas`.** No type or schema may be defined inside `apps/web` or `apps/backend`.
3. **Every DB query filters by `created_by`.** No query may return records belonging to a different user.
4. **Enum values stored as integers in DB; API responses include both the integer and the human-readable label.** The DAL maps int → label before returning to routes.
5. **Every mutation hook must declare an explicit `onError` handler that surfaces the error via the shadcn toast utility.** Silent or absent `onError` is forbidden.
6. **Every Drizzle schema change must be immediately followed by `pnpm db:generate` and `pnpm db:migrate`.** Migration files are committed in the same change as the schema modification.
7. **Default / canonical framework documents are stored as static string constants in `apps/backend/src/config/Constants.ts`, not in the database.** They are injected into AI system prompts as reference templates. The DB only stores user-customised versions saved after generation.
8. **`isCustomized` is the single source of truth for framework onboarding state.** Every user always has a `job_search_frameworks` row (seeded on sign-up). Row presence alone does not mean the user has set up their criteria — only `isCustomized === true` does. The onboarding redirect gate and the settings warning banner both read this flag, never row presence.
9. **Browser Run is rate-limited via D1, not KV or Durable Objects.** A single-row `browser_run_budget` table in the existing D1 database acts as the usage counter. KV was rejected due to eventual-consistency race conditions (two queue batches can both read a stale count). Durable Objects were rejected as overengineered for the volume (≤3,600 calls/month) and carry idle GB-second cost. D1 writes are serialized (SQLite), cost $0 at this scale, and add ≤5ms overhead against a 10-second Browser Run call. Shutdown threshold is 80% of the 10-hour/month Workers paid plan quota (28,800s). The gate lives in `InboundJobAlertHandler` — checked before each `scrapeJobUrl` call, elapsed time recorded after. Monthly reset is lazy (checked on every `isShutdown()` read), requiring no cron job.
