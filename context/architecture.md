...
- **Cloudflare D1 (primary database)**: All structured data — `users`, `companies`, `contacts`, `contact_history`, `notes`, `jobs`, `tasks`. Every table has a `created_by` column (Clerk user ID) as a scoping key. Multi-tenant by design.
  - `tasks`: Backs the Tasks page (formerly the placeholder Today page — `/today` was renamed to `/tasks`). Each row is a follow-up: `contactId` (nullable FK — schema allows non-contact tasks in future, but V1 only auto-creates contact follow-ups), `title`, `dueAt` (date-only `YYYY-MM-DD` text, not a full timestamp — comparisons are lexical string comparisons), `status` (IntEnum: 1=Pending, 2=Completed, 3=Missed), `note` (nullable), `completedAt` (nullable). Auto-created/kept in sync by `ContactsRepo.logContactHistory`: whenever a `*_sent` message is logged for a contact, `contacts.nextTouchDueAt` is set to `sentAt + TASK_FOLLOWUP_INTERVAL_DAYS` (7 days, `Constants.TASK_FOLLOWUP_INTERVAL_DAYS`) and `TasksRepo.syncFollowUpForContact` upserts the contact's one open (`Pending`) follow-up task to that due date — this is the first place `contacts.nextTouchDueAt` is ever actually written; previously the column existed but nothing set it. A daily cron (`triggers.crons` in `workers/api/wrangler.jsonc`, `TaskMissedSweepHandler` via the worker's new `scheduled()` export) flips `Pending` tasks with `dueAt < today` to `Missed`. Overdue rollup is a query-time concern, not a data concern: `TasksDAL.getTasksForDate` special-cases `date === today` to also pull in any `Pending`/`Missed` task with `dueAt < today` — overdue follow-ups always surface on the Today section (with an "Overdue by Nd" badge) rather than staying attached to their original past date. Past dates only ever show `Completed` tasks (via `GET /tasks/past`). The calendar-strip dot query (`POST /tasks/calendar`) only reports `hasPending`/`hasCompleted` per day — never `Missed` — since overdue tasks are never shown on their original day.
